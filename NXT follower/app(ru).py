...
def emergency_stop(*args):
    ...
    print("\n[СИСТЕМА] Беспроводная остановка робота")
    try:
    ...
    print("[СИСТЕМА] Робот безопасно остановлен.")
    os._exit(0)
...
try:
    print(f"Открытие беспроводного канала связи через {BLUETOOTH_PORT}...")
    print("Подождите...")
    
    ser = serial.Serial(BLUETOOTH_PORT, baudrate=57600, timeout=0.01)
    print("Беспроводной канал связи активирован")
    ...
    print(f"\n[ОШИБКА] Ошибка открытия порта {BLUETOOTH_PORT}: {e}")
    sys.exit()
...
if not camera.isOpened():
    print("[ОШИБКА] Ошибка открытия камеры")
    sys.exit()
...
try:
    while True:
        ...
        cv2.imshow("Zrenie robota", frame)
